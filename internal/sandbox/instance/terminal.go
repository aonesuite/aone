...
// reducing the number of SendInput calls (matching the CLI behavior).
...
func runTerminalSession(ctx context.Context, sb *sandbox.Sandbox) {
	...
	// Keep interval checks frequent enough for responsive terminal cleanup.
	go func() {
	...
	// Forward stdin to PTY using batched writer (10ms interval, matching the CLI behavior)
